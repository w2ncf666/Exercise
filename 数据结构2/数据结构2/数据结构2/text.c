...
pushstack(sqstack* s, int d)
{
	if (stackfull)
	{
		sqstack*p = (int*)realloc(s->base, (s->nowsize + ADDSIZE * sizeof(int)));//都要判断是否分配失败，上面初始化也是一样
		if (!p)
			return 0;
		s->base = p;
			return NULL;
...
Msort(int random[], int sorted[],int start,int end)//归并排序
{
	if (start == end)
...
radixsort(int arr[], int length)//基数排序
{
	int num=0,i,j=1;
